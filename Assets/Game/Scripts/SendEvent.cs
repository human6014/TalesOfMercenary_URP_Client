...
public class SendEvent : MonoBehaviour
{
    ...
    private void SendMoveUnitsToTargetPositionEvent()
    {
        ...
        Debug.Log("≈∏¿‘: " + (byte)EventCode.EsPlayerBuildingUpgradedMessage + "¡¬«•: " + (Vector3)content[0] + "id" + (byte)content[1] + "id" + (byte)content[2]  + "∫∏≥ø, √— ±Ê¿Ã;" + content.Length);
        PhotonNetwork.RaiseEvent((byte)EventCode.EsPlayerBuildingUpgradedMessage, content, raiseEventOptions, SendOptions.SendReliable);
    }
    private void GoodBye()
    {
        ...
        Debug.Log("≈∏¿‘: " + (byte)EventCode.EsPlayerSpawnedUnitMessage +  "id" + (byte)content[0] + "∫∏≥ø, √— ±Ê¿Ã;" + content.Length);
